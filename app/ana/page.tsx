...
export default async function AnaPage() {
  ...
  return (
    <div className="min-h-screen w-full">
      ...
      <main className="w-full max-w-full px-6 py-10">
        {/* Hero Section */}
        <div className="mb-12 text-center sm:text-left">
          <h2 className="text-3xl font-bold text-[#1B5E20] mb-2">Genel Bakış</h2>
          <p className="text-zinc-600">Tarım verilerine ve analiz araçlarına buradan ulaşabilirsiniz.</p>
        </div>

        {/* Navigation Cards with LED Effect */}
        <div className="grid md:grid-cols-3 gap-6 mb-16">
          <Link href="/bilgi" className="group relative overflow-hidden rounded-2xl bg-white p-8 shadow-sm hover:shadow-lg transition-all border border-zinc-100 btn-led">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
              <svg className="w-24 h-24 text-blue-600" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z" /></svg>
            </div>
          ...
          <Link href="/tani" className="group relative overflow-hidden rounded-2xl bg-white p-8 shadow-sm hover:shadow-lg transition-all border border-zinc-100 btn-led">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
              <svg className="w-24 h-24 text-green-600" fill="currentColor" viewBox="0 0 24 24"><path d="M10 4.5c0-.83-.67-1.5-1.5-1.5S7 3.67 7 4.5 7.67 6 8.5 6 10 5.33 10 4.5zM19 8c-1.66 0-3 1.34-3 3 0 1.31.84 2.41 2 2.83V17c0 1.66-1.34 3-3 3s-3-1.34-3-3v-4c0-1.66-1.34-3-3-3s-3 1.34-3 3v2h-2v-2c0-2.76 2.24-5 5-5s5 2.24 5 5v1.17c1.16-.42 2-1.52 2-2.83 0-1.66-1.34-3-3-3z" /></svg>
            </div>
          ...
          <Link href="/harita" className="group relative overflow-hidden rounded-2xl bg-white p-8 shadow-sm hover:shadow-lg transition-all border border-zinc-100 btn-led">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
              <svg className="w-24 h-24 text-amber-600" fill="currentColor" viewBox="0 0 24 24"><path d="M20.5 3l-.16.03L15 5.1 9 3 3.36 4.9c-.21.07-.36.25-.36.48V20.5c0 .28.22.5.5.5l.16-.03L9 18.9l6 2.1 5.64-1.9c.21-.07.36-.25.36-.48V3.5c0-.28-.22-.5-.5-.5zM15 19l-6-2.11V5l6 2.11V19z" /></svg>
            </div>
